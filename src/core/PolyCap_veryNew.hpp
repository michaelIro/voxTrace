...
// (and PolyCap.hpp), so the transmitted-weight output matches to ~1%.
//
...
public:
    KOKKOS_INLINE_FUNCTION PolyCap_veryNew() = default;
...
#ifndef __METAL_VERSION__
    PolyCap_veryNew(float posZ, float length,
    ...
    void print() const {
        ...
        printf("PolyCap_veryNew: %s L=%.3fcm rExt=(%.5f→%.5f)cm rCap=(%.7f→%.7f)cm "

"""
Beam cross-section comparison: original polycap C library vs PolyCap_new.hpp
vs PolyCap.hpp (current core).
Reads CSV files produced by Test3 (3way_lib_E*.csv, 3way_new_E*.csv, 3way_hpp_E*.csv).

Layout per energy:  3 rows x 5 columns
    Row 0 - polycap (C library, ground truth)   (forestgreen)
    Row 1 - PolyCap_new.hpp                     (steelblue)
    Row 2 - PolyCap.hpp (current core)          (firebrick)
  Columns - exit window | f/2 | focal | 3f/2 | 2f

Run from the project root after building and executing Test3:
    python3 src/tests/plot_beam_comparison_3way.py
"""
...
# ── geometry constants (must match Test-3way.cpp) ─────────────────────────────
...
ROWS = [
    ...
    ("new",     "PolyCap_new.hpp",   "steelblue"),
    ("verynew", "PolyCap_veryNew",   "darkorange"),
    ("hpp",     "PolyCap.hpp",       "firebrick"),
]

# ── plotting helpers ──────────────────────────────────────────────────────────

def load_beam(fname):
    """Return (plane_arr, x_mm_arr, y_mm_arr, w_arr) or None if file missing."""
...
def scatter_plane(ax, x_mm, y_mm, color, n_max=15_000):
    ...
    ax.scatter(x_mm, y_mm, s=0.8, alpha=0.35, color=color,
               linewidths=0, rasterized=True)
    rms = beam_rms_mm(x_mm, y_mm)
...
def main():
    ...
    # Base directory is three levels up from src/tests/
    ...
    for energy in ENERGIES:
        datasets = {}
        for key, _, _ in ROWS:
            fname = os.path.join(out_dir, f"3way_{key}_E{energy:.1f}.csv")
            datasets[key] = load_beam(fname)
        ...
        fig.suptitle(
            ...
            f"polycap (C lib) vs PolyCap_new.hpp vs PolyCap.hpp",
            fontsize=13, y=1.01,
        )

        # Per-column symmetric axis limits (same across all 3 rows for a fair comparison)
        col_lims = []
        ...
        plt.tight_layout()
        out = os.path.join(out_dir, f"beam_comparison_3way_E{energy:.1f}keV.png")
    ...
    if saved:
        ...
        print("\nNo beam CSV files found.  Run ./build/src/Test3 first.")
        sys.exit(1)
